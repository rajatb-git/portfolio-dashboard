...
import { Box, Toolbar, Button, Divider, Typography } from '@mui/material';
...
export default function Drawer({ collapsed, onToggle }: DrawerProps) {
  ...
  return (
    <>
      ...
      >
        <Toolbar sx={{ minHeight: '48px !important', gap: 1.5, px: '16px !important' }}>
          <Box sx={{ flexGrow: 1 }} />
      ...
      >
        ...
        <List sx={{ p: 0, flexGrow: 1 }}>
          {NAV_CONFIG.map(({ href, icon, text }) => (
            <SideNavItem key={text} href={href} icon={icon} text={text} collapsed={collapsed} />
          ))}
        </List>
