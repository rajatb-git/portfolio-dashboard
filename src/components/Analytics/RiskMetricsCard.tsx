import { Card, Chip, Divider, Grid, Skeleton, Stack, Typography } from '@mui/material';

import { RiskMetrics } from '@/api/analytics';
    ...
    <Card variant="outlined">
      ...
      {isLoading ? (
        ...
        <Typography sx={{ p: 3, textAlign: 'center', color: 'text.disabled', fontSize: '0.82rem' }}>
          Not enough data to calculate risk metrics. Load the Dashboard daily to build portfolio history.
        </Typography>
      ) : (
        <Grid container spacing={2.5} sx={{ p: 2 }}>
          ...
          <Grid size={{ xs: 6, sm: 4, md: 2 }}>
            <MetricItem
              ...
              subtext={metrics.sharpeRatio >= 1 ? 'Good' : metrics.sharpeRatio >= 0.5 ? 'Fair' : 'Poor'}
            />
          </Grid>
          <Grid size={{ xs: 6, sm: 4, md: 2 }}>
            <MetricItem
          ...
          <Grid size={{ xs: 6, sm: 4, md: 2 }}>
            <MetricItem
              ...
              subtext={metrics.maxDrawdownPeriod.from && metrics.maxDrawdownPeriod.to
