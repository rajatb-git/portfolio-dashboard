...
import { getHoldingsEarningsCalendar, getHoldingsEarningsResults } from '../controller/HoldingsEarningsController';
...
export const AnalyticsRouter = () => {
  ...
  router.get('/analytics/earnings-results', async (ctx) => {
    try {
      ctx.body = await getHoldingsEarningsResults();
      ctx.status = 200;
    } catch (error: any) {
      logger.log({ level: 'error', message: error.message, label: 'Holdings earnings results route' });
      ctx.body = errorBody('Failed to get earnings results', error.message);
      ctx.status = 400;
    }
  });
