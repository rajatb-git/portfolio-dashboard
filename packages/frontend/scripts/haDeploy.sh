...
yq e -i ".version = \"${NEW_VERSION}\"" package.json
yq e -i ".version = \"${NEW_VERSION}\"" package-lock.json


rm -rf HAapp
mkdir HAapp

cp -R public scripts src .dockerignore ingress.conf apparmor.txt config.yaml Dockerfile next-env.d.ts next.config.js package.json package-lock.json tsconfig.json README.md HAapp/
