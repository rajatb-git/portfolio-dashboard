      ...
      {isLoading ? (
        ...
        <Box>
          ...
          <MetricRow
            ...
            tooltip="Volatility relative to the S&P 500"
          />
          <MetricRow
            label="Price / Book"
            value={metrics.priceToBook?.toFixed(2) ?? '—'}
            tooltip="Price-to-book ratio (annual)"
          />
          <MetricRow
            label="ROE (TTM)"
            value={metrics.roeTTM ? `${metrics.roeTTM.toFixed(1)}%` : '—'}
            tooltip="Return on equity (trailing 12 months)"
          />
          <MetricRow
            label="Revenue Growth (YoY)"
            value={metrics.revenueGrowthTTMYoy ? `${metrics.revenueGrowthTTMYoy.toFixed(2)}%` : '—'}
            tooltip="TTM revenue growth vs prior year"
          />
          <MetricRow
            label="Avg Volume (10D)"
            value={metrics.avgVolume10Day ? `${metrics.avgVolume10Day.toFixed(2)}M` : '—'}
            tooltip="10-day average trading volume (millions)"
          />
          <MetricRow
            label="Short Interest"
            value={metrics.shortInterest ? `${metrics.shortInterest.toFixed(2)}M` : '—'}
            tooltip="Number of shares sold short (millions)"
          />
          <MetricRow
            label="Short Ratio (Days)"
            value={metrics.shortRatio?.toFixed(2) ?? '—'}
