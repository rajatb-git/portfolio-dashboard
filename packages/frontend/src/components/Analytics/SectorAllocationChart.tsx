    ...
    <Card variant="outlined" sx={{ height: '100%' }}>
      ...
      {isLoading ? (
        ...
        <Box sx={{ p: 3, textAlign: 'center' }}>
          <Typography sx={{ color: 'text.disabled', fontSize: '0.82rem' }}>No sector data available</Typography>
        </Box>
      ) : (
        <Box sx={{ p: 1 }}>
          <PieChart
          ...
          <Stack spacing={0.5} sx={{ px: 1.5, pb: 1.5 }}>
            {sectors.map((item, i) => (
              <Stack key={item.sector} direction="row" sx={{ alignItems: 'center', justifyContent: 'space-between' }}>
                <Stack direction="row" sx={{ alignItems: 'center' }} spacing={1}>
                  ...
                  />
                  <Typography sx={{ fontSize: '0.75rem', color: 'text.secondary' }}>{item.sector}</Typography>
                </Stack>
                <Stack direction="row" sx={{ alignItems: 'center' }} spacing={1}>
                  <Typography
                    sx={{
