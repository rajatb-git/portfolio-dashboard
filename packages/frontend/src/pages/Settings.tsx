    ...
    <Box sx={{ maxWidth: { xs: '100%', sm: 680 } }}>
      ...
      <SettingsSection title="Portfolio Value Tracker">
        ...
        {draftValueCalc.enabled && (
          <SettingRow label="Update Interval" description="How often to recalculate and save the portfolio value">
            <Select
              size="small"
      ...
      <SettingsSection title="Price Alert Monitor">
        ...
        {draftAlertMonitor.enabled && (
          <SettingRow label="Check Interval" description="How often the server polls live prices to evaluate alerts">
            <Select
              size="small"
        ...
        {draftNotif.mqtt.enabled && (
          <>
            ...
            <SettingRow label="Retain" description="Broker keeps the last message for new subscribers">
              <Switch checked={draftNotif.mqtt.retain} onChange={(_, checked) => setMqtt({ retain: checked })} />
            </SettingRow>
          </>
        )}
        <Stack direction="row" spacing={1} sx={{ justifyContent: 'flex-end', alignItems: 'center', px: 2, py: 1.5 }}>
